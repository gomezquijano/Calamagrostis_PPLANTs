...
This is a complete analysis pipeline of genetic structure of three species of *Calamagrostis* collected from the North West Territories and comercial cultivar samples of *C. canadensis*. For sample information use `calam_info.csv` and for more details about collections and samples used refer to https://github.com/gomezquijano/Calamagrostis_PPLANTs/Metadata 
...
## Loading SNPs and population information 

```{r}
calam_snps <- vcfR::read.vcfR("final.calamagrostis.snps.recode.vcf")

calam_gen <- vcfR::vcfR2genind(calam_snps)

#population information
ind_pops <- read.csv("calam_info.csv", header = T)
```
...
In this dataset we have also a couple of commercial **C. canadensis** individuals, we will eventually remove them but for now I want to leave them here to see how they look on a PCA
...
Calculating species pairwise FST using Nei's FST and Weir and Cockerham FST
```{r}
spec.matFst <- pairwise.neifst(calam_hier)
spec.matFst

spec.matWCFst <- pairwise.neifst(calam_hier)
spec.matWCFst
```

Lastly, let's create a Neighbour Joining tree to look at genetic distance between species.
