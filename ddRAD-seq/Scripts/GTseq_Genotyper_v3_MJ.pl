...
my $Raw_Reads = 0;
my %Null_Error = ();
my %NAratio == 5; #play with either 5 or 10
my %A1HOMratio == 100;
...
#Fix allele counts to non-zero number for division ratio calculation...
	if ($Allele1_Count{$loci} == 0) {$A1fix = 1} #MJ - changed this to 1 so that is just the inverse
	else {$A1fix = $Allele1_Count{$loci}}
	if ($Allele2_Count{$loci} == 0) {$A2fix = 1}
	...
	if ($Allele1_Count{$loci} < %NAratio and $Allele2_Count{$loci} <  %NAratio ) {$geno = "00"; $genoclass = "NA"; #MJ - each allele hsa to have at least 5 reads to pass
	elsif ($ratio >= %A1HOMratio) {$geno = "$allele1name{$loci}$allele1name{$loci}"; $genoclass = "A1HOM"; #Allele1 Homozygotes
		...
		#$HOM_CT = $HOM_CT + $Allele1_Count{$loci}; $BKGRD_CT = $BKGRD_CT + $Allele2_Count{$loci};}
	elsif ($ratio <= 1/%A1HOMratio) {$geno = "$allele2name{$loci}$allele2name{$loci}"; $genoclass = "A2HOM"; #Allele2 Homozygotes
	...
	else {$geno = "$allele1name{$loci}$allele2name{$loci}"; $genoclass = "HET";} #MJ - anything else that does not meet the conditions above is a heterozygote
